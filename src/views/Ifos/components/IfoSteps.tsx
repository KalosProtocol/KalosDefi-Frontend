...
import { getIXaloWeekDisplay } from 'views/Pools/helpers'
...
const Step1 = ({ hasProfile }: { hasProfile: boolean }) => {
  ...
  const weeksDisplay = getIXaloWeekDisplay(ceiling)

  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <Box>
      <Text>
        {t(
          'The number of iXALO equals the locked staking amount if the staking duration is longer than %weeks% weeks. If the staking duration is less than %weeks% weeks, it will linearly decrease based on the staking duration.',
          {
            weeks: weeksDisplay,
          },
        )}
  ...
  return (
    <CardBody>
      ...
      <Heading as="h4" color="secondary" mb="16px">
        {t('Lock XALO in the XALO pool')}
      </Heading>
      <Box>
        <Text mb="4px" color="textSubtle" small>
          {t(
            'The maximum amount of XALO you can commit to the Public Sale equals the number of your iXALO. Lock more XALO for longer durations to increase the maximum CAKE you can commit to the sale.',
          )}
        </Text>
        <TooltipText as="span" fontWeight={700} ref={targetRef} color="textSubtle" small>
          {t('How does the number of iXALO calculated?')}
        </TooltipText>
        <Text mt="4px" color="textSubtle" small>
          {t(
            'Missed this IFO? You will enjoy the same amount of iXALO for future IFOs if your locked-staking position is not unlocked.',
          )}
        </Text>
      </Box>
      {hasProfile && (
  ...
  return (
    <CardBody>
      ...
      <Text color="textSubtle" small>
        {t(
          'Please note that XALO in the fixed-term staking positions will remain locked and can not be used for committing to IFO sales. You will need a separate amount of CAKE in your wallet balance to commit to the IFO sales.',
        )}{' '}
        <br />
      </Text>
        ...
        return (
          <CardBody>
            ...
            <Text color="textSubtle" small mb="16px">
              {t('You’ll need an active KalosDeFi Profile to take part in an IFO!')}
            </Text>
            {renderAccountStatus()}
          </CardBody>
        )
      ...
      case 3:
        return (
          <CardBody>
            ...
            <Text color="textSubtle" small>
              {t(
                'After the IFO sales finish, you can claim any IFO tokens that you bought, and any unspent XALO tokens will be returned to your wallet.',
              )}
            </Text>
          </CardBody>
        )
