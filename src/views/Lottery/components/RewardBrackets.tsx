...
const RewardBrackets: React.FC<RewardMatchesProps> = ({ lotteryNodeData, isHistoricRound }) => {
  ...
  useEffect(() => {
    if (lotteryNodeData) {
      ...
      const xaloToBurn = feeAsPercentage.div(100).times(new BigNumber(amountCollectedInXalo))
      const amountLessTreasuryFee = new BigNumber(amountCollectedInXalo).minus(xaloToBurn)
      setState({
        isLoading: false,
        xaloToBurn,
  ...
  const getXaloRewards = (bracket: number) => {
  ...
  const { isLoading, countWinnersPerBracket, xaloToBurn } = state
      ...
      <RewardsInner>
        {rewardBrackets.map((bracketIndex) => (
          <RewardBracketDetail
            ...
            xaloAmount={!isLoading && getXaloRewards(bracketIndex)}
            numberWinners={!isLoading && countWinnersPerBracket[bracketIndex]}
            isHistoricRound={isHistoricRound}
            isLoading={isLoading}
          />
        ))}
        <RewardBracketDetail rewardBracket={0} xaloAmount={xaloToBurn} isBurn isLoading={isLoading} />
      </RewardsInner>
    </Wrapper>
  )
}
