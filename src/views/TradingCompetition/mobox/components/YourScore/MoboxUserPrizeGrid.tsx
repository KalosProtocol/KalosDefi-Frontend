...
const MoboxUserPrizeGrid: React.FC<{ userTradingInformation?: UserTradingInformation }> = ({
  ...
  const { userRewardGroup, userXaloRewards, userMoboxRewards, userPointReward, canClaimMysteryBox, canClaimNFT } =
    userTradingInformation
  const { xaloReward, moboxReward, dollarValueOfTokensReward } = useMoboxCompetitionRewards({
    userXaloRewards,
    userMoboxRewards,
  })
        ...
        <tr>
          <BoldTd>
            <Flex flexDirection="column">
              <Text bold>{xaloReward.toFixed(4)} XALO</Text>
