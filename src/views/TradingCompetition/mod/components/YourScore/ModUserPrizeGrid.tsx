...
const ModUserPrizeGrid: React.FC<{ userTradingInformation?: UserTradingInformation }> = ({
  ...
  const { userRewardGroup, userXaloRewards, userDarRewards, userPointReward, canClaimNFT } = userTradingInformation
  const canClaimSpecialNFT = useCanClaimSpecialNFT()
  const { xaloReward, darReward, dollarValueOfTokensReward } = useModCompetitionRewards({
    userXaloRewards,
    userDarRewards,
  })
